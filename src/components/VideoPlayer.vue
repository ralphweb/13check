...
export default {
  ...
  computed: {
    ...
    calculatedmuted: {
      get() {
        return this.globalmuted ? true : this.muted ? true : false;
      },
    },
  },
  mounted() {
        ...
        if (!that.live && now.diff(query, "minutes") > 15) {
          ...
          if (
            ...
            (that.currentFile == null ||
              query.isBefore(that.currentStart) ||
              query.isAfter(that.currentEnd))
          ) {
            that.loading = true;
            that.localTime = that.time;
            axios
              .get(
                "https://13checkqa.ingenieriac13.cl/vod/video/" +
                  that.ip +
                  "/" +
                  query.format("YYYY-MM-DD[_]HH-mm-ss")
              )
              .then((response) => {
                that.loading = false;
          ...
          that.player.src({
            ...
            src:
              "https://13checkqa.ingenieriac13.cl/stream/" +
              that.ip +
              "/index.m3u8",
          });
        }
      ...
      handler(newFile) {
        ...
        if (newFile && that.player != null) {
          that.player.src({
            type: "video/mp4",
            src:
              "https://13checkqa.ingenieriac13.cl/vod/getvideo/" +
              that.ip +
              "/" +
              encodeURIComponent(newFile),
          });
