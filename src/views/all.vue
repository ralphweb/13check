<template>
  <div v-bind:class="{ 'transition fadeInDown': true }">
    <div class="row fadeIn first main">
      <signal
        :ref="'signal'"
        :index="i"
        v-bind:key="i"
    ...
    <div class="row fadeIn second signals">
      <signal
        :ref="'signal'"
        :index="i + 1"
        v-bind:key="i"
...
export default {
  ...
  methods: {
    ...
    onSwipeDown() {
      var that = this;
      that.current =
        that.current + 1 < that.signalsCurrent.length ? that.current + 1 : 0;
      that.$scrollTo("#video" + that.signalsCurrent[that.current].idRating);
    },
  },
  computed: {
